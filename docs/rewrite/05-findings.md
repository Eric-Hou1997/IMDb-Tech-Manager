...
依据：[原始验证器](../../macos/update.go)；[Tauri 官方实现](https://raw.githubusercontent.com/tauri-apps/plugins-workspace/v2/plugins/updater/src/updater.rs)。TCM 旧更新入口也要单独对照安装方式和资产选择，不假定继承 ITM 的密钥。
...
旧 `macos/engine/mac-engine.py:4443` 在 `legacy_cleanup_mode=strict` 下拒绝无原始备份的推断清理；`inferred` 允许继续进入后续流程。新版保留配置字段供迁移识别，但不通过文本相似或规则重新生成结果推断删除权限。任何自动写入均要求权威归属，未知或冲突时 `unsafe-skip`。这是依据仓库 NFO 安全规则确定的行为差异，不把可能侵入 External 标签的旧路径作为兼容目标；历史条目需要经核对的 sidecar/嵌入 manifest 迁移或用户显式 Inspector 操作。配置导入应提示该差异，不得静默启用推断清理。

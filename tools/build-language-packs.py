...
EMBEDDED_CATALOG_PATH = ROOT / "macos" / "language_catalog.json"
FIXED_ZIP_TIME = (1980, 1, 1, 0, 0, 0)
VERSION_RE = re.compile(r"^v(\d+)\.(\d+)\.(\d+)$")
EXPECTED_LOCALES = {"fr-FR", "ru-RU", "ja-JP", "es-ES", "th-TH"}
EXPECTED_SECTIONS = ["web", "core", "engine", "native"]
NATIVE_MESSAGES = {
    "About IMDb Tech Manager", "Quit IMDb Tech Manager", "IMDb Tech Manager Could Not Start",
    "Quit", "OK", "Confirm", "Cancel",
}
TARGET_SCRIPT_RE = {
...
ALLOWED_IDENTICAL = {
    "AI", "API", "CRLF", "Emby", "GitHub", "HTTP", "IMDb", "IMDb ID", "IMDb Tech Manager",
    "JSON", "LF", "NFO", "OK", "Optimal", "Qwen", "SHA-256", "UTF-8", "URL", "WebKit",
    "XML", "ZIP", "macOS", "—",
}
...
def required_messages() -> dict[str, set[str]]:
    web_source = (ROOT / "macos" / "web" / "index.html").read_text(encoding="utf-8")
    english_ui = web_source.split("const ENGLISH_UI=Object.freeze({", 1)[1].split("});\nconst UI_LOCALES", 1)[0]
    web = {
        decode_js_string(match.group(2)).strip()
        for match in re.finditer(r"'((?:\\.|[^'\\])*)'\s*:\s*'((?:\\.|[^'\\])*)'", english_ui)
    }
    localization = (ROOT / "macos" / "localization.go").read_text(encoding="utf-8")
    core = {
        json.loads('"' + match.group(2) + '"').strip()
        for match in re.finditer(r'\{"((?:[^"\\]|\\.)*)",\s*"((?:[^"\\]|\\.)*)"\}', localization)
    }
    return {"web": web, "core-engine": core, "native": NATIVE_MESSAGES}
...
def require_complete_translation(source: dict, locale: str, required: dict[str, set[str]]) -> None:
    ...
    if missing:
        ...
        raise ValueError(f"{locale}: incomplete release translation coverage ({summary})")


def load_catalog() -> dict:
    catalog = json.loads(CATALOG_PATH.read_text(encoding="utf-8"))
    if catalog.get("schema") != 1 or catalog.get("product") != "itm":
    ...
    for locale, descriptor in languages.items():
        ...
        if descriptor.get("asset") != f"ITM-Language-{locale}-r{revision}.zip":
...
def build_pack(catalog: dict, locale: str, descriptor: dict, required: Optional[dict[str, set[str]]] = None) -> tuple[bytes, str]:
    ...
    section_files = {
        f"{section}.json": json_bytes(messages)
        for section, messages in encoded_sections.items()
    }
    manifest = {
        "schema": 1,
        ...
        "files": {
            ...
            for name, payload in sorted(section_files.items())
        },
    }
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, help="write release assets to this directory")
    parser.add_argument("--changed-only", action="store_true", help="build only packs first published with this app version")
    parser.add_argument("--update-catalog", action="store_true", help="mechanically refresh catalog hashes")
    parser.add_argument("--app-version", help="require an exact catalog app version, such as v4.1.0")
    parser.add_argument("--require-complete", action="store_true", help="reject packs that do not cover every registered release string")

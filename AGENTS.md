...
- Every localized README is a complete, section-for-section translation of the current full README, not a summary or shortened overview. Preserve all capabilities, workflows, architecture, safety rules, installation/update guidance, status, roadmap, contribution, license, disclaimer, links, images, examples, and code blocks; do not omit or merge content merely to reduce translation work.
- README synchronization is an all-locales completion gate, not a best-effort documentation task. Before reporting completion, committing, pushing, tagging, or releasing, verify every supported locale against the full source document: the section inventory and order, heading levels, lists and roadmap checkboxes, code fences and examples, images, links, version facts, and legal-document targets must all remain present. A materially shorter, source-language-filled, placeholder, partial, or summarized locale is incomplete and blocks closeout for the entire README set.
- Translate README and public legal-document prose directly as part of Codex's stewardship. Do not invoke a local or third-party translation model, silently reuse machine-generated drafts, or leave untranslated source-language prose in a localized variant unless the maintainer explicitly authorizes that translation method. Stable product names, protocol keys, filenames, commands, code, and other protected technical terms remain unchanged where appropriate.
- Never claim that multilingual documentation is complete while any supported locale is missing or abridged. Report the exact incomplete locale inventory during ongoing work, keep partial work uncommitted, and continue until the whole synchronized set passes parity and link validation.
- Apply the same stewardship to public legal documents. Keep the default Simplified Chinese `PRIVACY.md` and `TERMS.md` at the repository root, keep all translated variants under `docs/legal/`, and place the complete language selector at the top of every variant. Every release must check their facts, supported-language inventory, links, and translation parity together with the READMEs.
- If the maintainer manually changes any README, privacy policy, or terms variant, treat that locale's change as intentional source material and propagate its meaning to every other locale before release.

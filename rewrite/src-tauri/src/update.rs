...
fn public_key() -> Result<&'static str> {
    if crate::PRODUCT == "ITM" {
        Ok(include_str!("../core/assets/ota-public-key.txt").trim())
    } else {
...
pub async fn update_check(id: String, app: tauri::AppHandle) -> Result<UpdateProgress> {
    ...
    match app.state::<Desktop>().store.operation_result(&id) {
        Ok(OperationResult::Update(value)) => return Ok(value),
        Ok(_) => {
            return Err(error(
                "operation-conflict",
...
pub async fn update_install(id: String, app: tauri::AppHandle) -> Result<UpdateProgress> {
    ...
    let result = async {
        ...
        #[cfg(not(windows))]
        {
            ...
            let candidate = product_core::install::stage(
                &journal,
                &id,
                &target,
                &bytes,
            ...
            product_core::install::validate_candidate(
                &candidate,
                &app.config().identifier,
                &pending.artifact.version,
                &pending.identity.arch,
            )?;
            #[cfg(target_os = "macos")]
            {
                let status = std::process::Command::new("/usr/bin/codesign")
                    ...
                    .status()
                    .map_err(|e| error("update-code-signature", e))?;
                if !status.success() {
                    return Err(error(
                        "update-code-signature",
                        "Application code signature is invalid",
                    ));
                }
            }
...
pub fn frontend_healthy(app: &tauri::AppHandle) -> Result<()> {
    let desktop = app.state::<Desktop>();
    let store = &desktop.store;
    let value = store.preferences("update-state")?;
    if value.get("operation_id").is_none() {
        return Ok(());
    }
    let mut progress: UpdateProgress = serde_json::from_value(value)?;
    if matches!(
            ...
            == Some(app.package_info().version.to_string().as_str())
        {
            #[cfg(not(windows))]
            {
                let journal = app
                    ...
                    .map_err(|e| error("update-data-directory", e))?
                    .join("updates")
                    .join(format!("{}-application.json", progress.operation_id));
                if journal.exists() {
                    let recovered = product_core::install::recover(&journal)?;
                    if recovered.phase != "installed-awaiting-health" {
                        return Err(error(
                            "update-recovery-required",
                            "Application replacement has not committed",
                        ));
                    }
                }
            }
            progress.phase = "verified".into();
            progress.error = None;
        } else {
            progress.phase = "recovery-required".into();
            progress.error=Some(error("update-version-not-installed","Restarted version does not match the requested update; previous data backup retained"));
        }
        record(app, &progress)?;
    } else if matches!(
        progress.phase.as_str(),

...
#[test]
fn database_snapshot_is_immutable_and_installation_freezes_normal_writes() {
    ...
    store
        .save_preference(
            "custom-prompt",
            &serde_json::json!({"text":"原始自定义提示词"}),
        )
        .unwrap();
    store.freeze_for_update().unwrap();
    assert_eq!(
        store
            .save_preference("custom-prompt", &serde_json::json!({"text":"changed"}))
            .unwrap_err()
            .code,
        "update-in-progress"
    );
    let backup = root.join("before-update.sqlite");
    store.snapshot_database(&backup).unwrap();
    assert_eq!(
        store.snapshot_database(&backup).unwrap_err().code,
        "update-backup-exists"
    );
    store.unfreeze_after_update();
    store
        .save_preference("custom-prompt", &serde_json::json!({"text":"new version"}))
        .unwrap();
    let previous = Store::open(&backup).unwrap();
    assert_eq!(
        previous.preferences("custom-prompt").unwrap()["text"],
        "原始自定义提示词"
    );
}

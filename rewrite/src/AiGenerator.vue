...
<template>
 <section aria-label="标签生成" class="generator">
  ...
  <section v-if="record" aria-label="AI 结果"><p>{{ record.phase }} · {{ record.cached?'缓存命中':'Provider 请求' }} · {{ record.title }} {{ record.year }}</p>
   ...
   <div class="actions"><button v-if="record.phase==='review-ready'" :disabled="busy||blocked" @click="prepare">检查 AI 结果并预览写入</button><button :disabled="busy||blocked||!configured" @click="generate(true)">强制重建 AI 候选</button><button v-if="record.error" :disabled="busy||blocked||!configured" @click="generate(false,true)">明确重试失败项</button></div>
  </section>

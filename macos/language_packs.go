...
const (
	...
	languagePackProduct   = "itm"
	languagePackRepoPath  = "Eric-Hou1997/IMDb-Tech-Manager"
)
...
func mustLoadLanguageCatalog(b []byte) LanguageCatalog {
	...
	for language, descriptor := range catalog.Languages {
		expectedAsset := fmt.Sprintf("ITM-Language-%s-r%d.zip", language, descriptor.Revision)
...
func decorateLanguageOptions(options []LanguageOption) {
	...
	for index := range options {
		...
		option.Downloadable = published
		option.Revision = descriptor.Revision
		option.ReleasedWith = descriptor.ReleasedWith
...
func languagePackMessage(language, section, id string) (string, bool) {
	if _, _, installed := installedLanguagePack(language); !installed {
		return "", false
	}
	languagePackMu.RLock()
	...
	if !cached {
		...
		for _, name := range []string{"web", "core", "engine", "native"} {
...
func languagePackDownloadURL(descriptor LanguagePackDescriptor) (string, error) {
	...
	wantPrefix := "ITM-Language-"
	if !strings.HasPrefix(descriptor.Asset, wantPrefix) || !strings.HasSuffix(descriptor.Asset, ".zip") || strings.Contains(descriptor.Asset, "/") {
...
func installLanguagePack(language string) error {
	...
	request.Header.Set("User-Agent", "IMDb-Tech-Manager/"+appVersion)
...
func extractLanguagePack(payload []byte, language string, descriptor LanguagePackDescriptor) error {
	...
	allowed := map[string]bool{"manifest.json": true, "web.json": true, "core.json": true, "engine.json": true, "native.json": true}
	...
	for _, section := range []string{"web", "core", "engine", "native"} {
		var messages map[string]string
		name := section + ".json"
...
func handleLanguagePacks(w http.ResponseWriter, r *http.Request) {
	languageForErrors := normalizedLanguage(loadSettings().Language)
	...
	writeJSON(w, map[string]interface{}{"ok": true, "language": language, "languages": supportedLanguages()})
}
...
func languagePackSection(language, section string) (map[string]string, bool) {
	...
	if _, ok := languagePackMessage(language, section, "__load__"); ok {
		// A real message named __load__ is harmless; the call above only primes
		// the immutable in-memory cache.
	}
	languagePackMu.RLock()
	defer languagePackMu.RUnlock()
...
var traditionalChineseReplacer = strings.NewReplacer(
	"简体中文", "簡體中文", "语言", "語言", "设置", "設定", "当前", "目前", "媒体库", "媒體庫",
	"电影", "電影", "电视剧", "電視劇", "标题", "標題", "标签", "標籤", "状态", "狀態",
	"选择", "選擇", "显示", "顯示", "检查", "檢查", "错误", "錯誤", "启动", "啟動",
	"关闭", "關閉", "下载", "下載", "应用", "應用程式", "目录", "目錄", "路径", "路徑",
	"任务", "任務", "完成", "完成", "失败", "失敗", "刷新", "重新整理", "确认", "確認",
	"删除", "刪除", "恢复", "復原", "缓存", "快取", "文件", "檔案", "后台", "背景",
	"自动", "自動", "运行", "執行", "连接", "連線", "支持", "支援", "网络", "網路",
	"用户", "使用者", "数据", "資料", "读取", "讀取", "写入", "寫入", "总数", "總數",
)

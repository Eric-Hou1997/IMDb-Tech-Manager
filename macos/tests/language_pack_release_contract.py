...
assert CATALOG["product"] == "itm"
assert CATALOG["app_version"] == "v4.1.0"
assert (ROOT / "macos" / "language_catalog.json").read_bytes() == (
    ROOT / "language-packs" / "catalog.json"
).read_bytes()

with tempfile.TemporaryDirectory(prefix="itm-language-pack-") as temporary:
...
with tempfile.TemporaryDirectory(prefix="itm-language-pack-immutable-") as temporary:
    checkout = Path(temporary)
    (checkout / "tools").mkdir()
    (checkout / "macos").mkdir()
    shutil.copy2(BUILDER, checkout / "tools" / BUILDER.name)
    shutil.copytree(ROOT / "language-packs", checkout / "language-packs")
    shutil.copy2(ROOT / "macos" / "language_catalog.json", checkout / "macos" / "language_catalog.json")
...
print("OK deterministic ITM language-pack release contract")

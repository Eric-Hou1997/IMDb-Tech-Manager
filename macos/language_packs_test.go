...
func testLanguageArchive(t *testing.T, product, locale string, descriptor LanguagePackDescriptor) []byte {
	t.Helper()
	files := map[string][]byte{"web.json": []byte(`{"legacy.test":"Web"}`), "core.json": []byte(`{"legacy.test":"Core"}`), "engine.json": []byte(`{}`), "native.json": []byte(`{}`)}
	...
	for _, name := range []string{"manifest.json", "web.json", "core.json", "engine.json", "native.json"} {
...
func TestLanguagePackDownloadInstallAndExactCatalogBinding(t *testing.T) {
	...
	descriptor := LanguagePackDescriptor{Revision: 7, ReleasedWith: "v4.1.0", Asset: "ITM-Language-fr-FR-r7.zip", CatalogSchema: 1, MessageSetHash: strings.Repeat("1", 64)}
	...
	languagePackHTTPClient = &http.Client{Transport: languageRoundTripper(func(request *http.Request) (*http.Response, error) {
		if !strings.Contains(request.URL.Path, "/releases/download/v4.1.0/ITM-Language-fr-FR-r7.zip") {
...
func TestLanguagePackRejectsWrongProduct(t *testing.T) {
	...
	descriptor := LanguagePackDescriptor{Revision: 1, ReleasedWith: "v4.1.0", Asset: "ITM-Language-ja-JP-r1.zip", CatalogSchema: 1, MessageSetHash: strings.Repeat("1", 64)}
...
func TestLanguagePackRejectsSymlinkedInstallRoot(t *testing.T) {
	outside := t.TempDir()
	link := filepath.Join(t.TempDir(), "Language Packs")
	if err := os.Symlink(outside, link); err != nil {
		t.Fatal(err)
	}
	languagePackRootOverride = link
	t.Cleanup(func() { languagePackRootOverride = "" })
	descriptor := LanguagePackDescriptor{Revision: 1, ReleasedWith: "v4.1.0", Asset: "ITM-Language-ja-JP-r1.zip", CatalogSchema: 1, MessageSetHash: strings.Repeat("1", 64)}
	payload := testLanguageArchive(t, languagePackProduct, "ja-JP", descriptor)
	if err := extractLanguagePack(payload, "ja-JP", descriptor); err == nil || !strings.Contains(err.Error(), "目录不安全") {
		t.Fatalf("symlinked language-pack root was not rejected: %v", err)
	}
